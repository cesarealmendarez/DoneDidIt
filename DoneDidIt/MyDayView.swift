...
struct MyDayView: View {
    ...
    @State var listName: String = "My Day"
    ...
    var body: some View {
        ...
        let myDayTasks = getMyDayTasks(viewModel: viewModel)
        
        // Today Timestamp
        let todayDateFormatter = DateFormatter()
        todayDateFormatter.dateFormat = "EEEE, MMMM d"
        
        let todayTimestampFormatted = todayDateFormatter.string(from: Date())
            ...
            VStack(spacing: 0) {
                ScrollView(.vertical, showsIndicators: false) {
                    HStack(alignment: .center) {
                        VStack(spacing: 5) {
                            TextField("Untitled List", text: $listName).foregroundColor(.orange).font(.largeTitle).fontWeight(.black).tint(.orange)
                                .disabled(true)
                            HStack(spacing: 0) {
                                Text("\(todayTimestampFormatted)").foregroundColor(.orange).font(.title3).fontWeight(.light)
                                Spacer()
                            }
                        }
                        Spacer()
                    ...
                    VStack(alignment: .center, spacing: 15) {
                        ForEach(myDayTasks) { task in
                            let list = getList(viewModel: viewModel, listID: task.taskListID)
                            
                            NavigationLink(destination: TaskView(viewModel: viewModel, listID: task.taskListID, taskID: task.taskID, taskTitle: task.taskTitle, parentView: "MyDayView")) {
                                HStack(alignment: .center, spacing: 15) {
                                    if(task.taskCompleted == true) {
                                        Button(action: { viewModel.markTaskIncomplete(listID: task.taskListID, taskID: task.taskID) }) {
                                            Image(systemName: "checkmark.circle.fill").foregroundColor(.orange).font(.title)
                                        }
                                    } else {
                                        Button(action: {
                                    ...
                                    VStack(alignment: .leading, spacing: 5) {
                                        ...
                                        TaskDetailBarView(task: task, accent: .orange, parentView: "MyDayView", parentListName: list.listName)
                                    }
                    ...
                    VStack(spacing: 30) {
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: 25) {
                                ...
                                }.tint(.orange).buttonStyle(.bordered).buttonBorderShape(.capsule)
                                    ...
                                    NavigationStack {
                                        ScrollView(.vertical, showsIndicators: false) {
                                            VStack {
                                                ...
                                                NavigationLink(destination: ListView_AddTaskDueDatePickerView(accent: .orange, addTaskDueDateSet: self.$addTaskDueDateSet, addTaskDueDateSheetOpen: self.$addTaskDueDateSheetOpen, addTaskDueDateTimestamp: self.$addTaskDueDateTimestamp, addTaskDueDateTimestampFormatted: self.$addTaskDueDateTimestampFormatted)) {
                                                        HStack {
                    ...
                    VStack {
                        Button(action: {
                            ...
                            HStack {
                                ...
                                Text("Add Task").foregroundColor(.orange)
                                Spacer()
                ...
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(action: { presentationMode.wrappedValue.dismiss() }) {
                        HStack {
                            ...
                            Text("Lists").foregroundColor(.orange)
                        }
                    }
                }
