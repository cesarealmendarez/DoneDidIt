...
struct PinnedView: View {
    ...
    @State var listName: String = "Pinned"
    ...
    var body: some View {
        ...
        let pinnedTasks = getPinnedTasks(viewModel: viewModel)
            ...
            VStack(spacing: 0) {
                ScrollView(.vertical, showsIndicators: false) {
                    HStack(alignment: .center) {
                        VStack(spacing: 5) {
                            TextField("Untitled List", text: $listName).foregroundColor(.red).font(.largeTitle).fontWeight(.black).tint(.red)
                                .disabled(true)
                        }
                        Spacer()
                    }.padding(.top, 20)
                    ...
                    VStack(alignment: .center, spacing: 15) {
                        ForEach(pinnedTasks) { task in
                            ...
                            NavigationLink(destination: TaskView(viewModel: viewModel, listID: task.taskListID, taskID: task.taskID, taskTitle: task.taskTitle, parentView: "PinnedView")) {
                                HStack(alignment: .center, spacing: 15) {
                                    if(task.taskCompleted == true) {
                                        Button(action: { viewModel.markTaskIncomplete(listID: task.taskListID, taskID: task.taskID) }) {
                                            Image(systemName: "checkmark.circle.fill").foregroundColor(.red).font(.title)
                                        }
                                    } else {
                                        Button(action: {
                                    ...
                                    VStack(alignment: .leading, spacing: 5) {
                                        ...
                                        TaskDetailBarView(task: task, accent: .red, parentView: "MyDayView", parentListName: list.listName)
                                    }
                    ...
                    VStack(spacing: 30) {
                        ...
                        ScrollView(.horizontal, showsIndicators: false) {
                            HStack(spacing: 25) {
                                ...
                                }.tint(.red).buttonStyle(.bordered).buttonBorderShape(.capsule)
                                    ...
                                    NavigationStack {
                                        ScrollView(.vertical, showsIndicators: false) {
                                            VStack {
                                                ...
                                                NavigationLink(destination: ListView_AddTaskDueDatePickerView(accent: .red, addTaskDueDateSet: self.$addTaskDueDateSet, addTaskDueDateSheetOpen: self.$addTaskDueDateSheetOpen, addTaskDueDateTimestamp: self.$addTaskDueDateTimestamp, addTaskDueDateTimestampFormatted: self.$addTaskDueDateTimestampFormatted)) {
                                                        HStack {
                                    ...
                                    NavigationStack {
                                        ScrollView(.vertical, showsIndicators: false) {
                                            VStack {
                                                ...
                                                NavigationLink(destination: ListView_AddTaskReminderPickerView(accent: .red, addTaskReminderSet: self.$addTaskReminderSet, addTaskReminderSheetOpen: self.$addTaskReminderSheetOpen, addTaskReminderTimestamp: self.$addTaskReminderTimestamp, addTaskReminderTimestampFormatted: self.$addTaskReminderTimestampFormatted)) {
                                                        HStack {
                                                            Image(systemName: "calendar").foregroundColor(.white)
                    ...
                    VStack {
                        Button(action: {
                            ...
                            HStack {
                                ...
                                Text("Add Task").foregroundColor(.red)
                                Spacer()
                ...
                ToolbarItem(placement: .navigationBarLeading) {
                    Button(action: { presentationMode.wrappedValue.dismiss() }) {
                        HStack {
                            ...
                            Text("Lists").foregroundColor(.red)
                        }
                    }
                }
